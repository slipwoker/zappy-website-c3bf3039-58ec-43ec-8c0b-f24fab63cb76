    ...
    contactForm.addEventListener('submit', function(e) {
      ...
      inputs.forEach(function(input) {
        const errorElement = input.parentElement.querySelector('.error-message') || 
                           input.nextElementSibling && input.nextElementSibling.classList.contains('error-message') ? 
        ...
        if (input.type === 'tel' && input.value.trim() && input.hasAttribute('required')) {
          ...
          if (!phonePattern.test(input.value.trim()) || input.value.trim().length < 10) {
            isValid = false;
            input.classList.add('error');
            const error = document.createElement('span');
            error.className = 'error-message';
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "ניהוליסטים משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: 'c3bf3039-58ec-43ec-8c0b-f24fab63cb76',
                    name: data.name || '',
                    email: data.email || '',
